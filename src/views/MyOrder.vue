            ...
            <div v-if="hasOrders">
                <div class="border-bold px-3 py-5" v-for="(order,i) of orders" :key="i">
                    ...
                    <div class="row border-0 pt-3 mx-5">
                        ...
                        <div class="col two">{{!!order.deliver_time?new Date(order.deliver_time).toLocaleString():"————"}}</div>
                        <div class="col two">
                            {{!!order.received_time?new Date(order.deliver_time).toLocaleString():"————"}}
                        </div>
                        <div class="col">{{order.status|showStatus}}</div>
                        <div class="col two">
                            <button class="mr-2" :disabled="order.status!==1" @click="updateOrder(order.order_id,i,4)">取消订单</button>
                            <button :disabled="order.status===4||order.status===3" @click="updateOrder(order.order_id,i,3)">确认收货</button>
                        </div>
                    </div>
...
export default {
    ...
    methods:{
        ...
        getProduct(){
            ...
            funs.getProduct(url,obj,callback);
        },
        //取消订单，更新订单状态
        updateOrder(order_id,i,status){
            ...
            funs.updateOrder(url,obj,callback);
        }
    },
    filters:{
        showStatus(val){
